...
2. **Fixed internal navigation links.** The originals linked page-to-page as `contact.html`,
   ...
   each page's slug. Links were rewritten to `/`, `/contact`, `/estimate-request`, `/faq`, `/financing`,
   `/gallery`, `/service-areas`, `/services-content`, `/storm-damage-insurance-claims`. **This only works if you set each
...
6. **Slugs corrected against the real live site.** The page source from bigbossroofing.com showed
   3 of my assumed slugs were wrong: `/services` → `/services-content`, `/estimate` →
   `/estimate-request`, `/storm-damage` → `/storm-damage-insurance-claims`. All internal links have
   been updated to match.

## Note on animations
The live homepage today is built with Squarespace's native page builder (Fluid Engine blocks), and
its buttons/text/shapes have hover and scroll animations attached through Squarespace's own
per-block **Animation** panel (visible in the page source as `data-block-animations` JSON — e.g.
buttons scale to 110% and lift on hover with a spring easing, the headline scales on hover, the
site-wide scroll-fade setting is also on). That animation system only runs on native Squarespace
blocks; it does not reach into a Code Block's HTML, so none of these effects apply to the code-block
pages in this folder. If you want equivalent motion here, it has to be hand-written CSS/JS inside
each fragment (hover transitions, `:hover` transforms, etc.) — happy to add that once you confirm
which effects to match.
...
| Services | `/services-content` | Roofing Services in DFW \| BigBoss Roofing LLC | Roof repairs, full replacements, inspections, metal & flat roofing, gutters, and storm damage services for homes and businesses across Dallas-Fort Worth. |
| Storm Damage | `/storm-damage-insurance-claims` | Storm Damage & Insurance Claims \| BigBoss Roofing LLC | Hail and wind damage inspections, photo documentation, and insurance claim assistance for DFW homeowners. Fast response for roof leaks and storm repairs. |
...
| Estimate | `/estimate-request` | Request a Free Roofing Estimate \| BigBoss Roofing LLC | Get a free, no-obligation roof estimate from BigBoss Roofing LLC. Tell us about your project and we'll follow up fast, serving all of DFW. |
...
| estimate.html | `/estimate-request` | f4f79458-estimate.html |
...
| services.html | `/services-content` | f171e0d6-services.html |
| storm-damage.html | `/storm-damage-insurance-claims` | d7a016fa-stormdamage.html |
