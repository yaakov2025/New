...
test("renders install page markup", () => {
  ...
  assert.match(html, /Add Attic Vent Calc to your/);
...
test("renders the manifest with the per-app name", () => {
  ...
  const fromSite = JSON.parse(
    renderWebManifest("atticventcalculator.grok.me", { title: "Attic Vent Calc" }),
  );
  assert.equal(fromSite.name, "Attic Vent Calc");
  assert.equal(fromSite.short_name, "Attic Vent Calc");
});
